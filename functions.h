...
class MKE{
	protected:
		...
		int count_inter_point;//количество внутренних узлов
		...
		int *interpoint;//внутреннии узлы
	...
	public:
		...
		void read_inter_point(char *filename);//Считывание номеров внутренних узлов
		...
		void LOS_LU_sq();// ЛОС LU(sq) факторизации
		void calc_start_values();//вычисление стартовых значений
		void dec_LU_sq();//LU(sq) факторизация
		TYPE dec_calc_elem(int i, int j, int current_elem);//факторизация внедиагонального элемента
		TYPE dec_calc_diag(int j, int current_elem);//факторизация диагонального элемента
		void direct(TYPE *in,TYPE *out);//прямой ход
		void reverse(TYPE *in,TYPE *out);//Обратный ход
		void write_result(int total,TYPE nevyazka);//вывод результата
		void mul_matrix_vector(TYPE *v);//умножение матрицы на вектор
